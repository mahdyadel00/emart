...
<a href="#" class="btn color-white waves-effect waves-light btn-primary edit" title="{{_i('Edit')}}" data-toggle="modal" data-target="#edit-modal" data-url='{{route('ticket.category.update', $id)}}' data-id='{{$id}}' data-lang_id='{{$lang_id}}' data-name='{{$name}}' data-description='{{$description}}' data-image='{{$image}}' data-color='{{$color}}'>
    <i class="ti-pencil-alt"></i> {{_i('Edit')}}
</a>
<a class="btn color-white waves-effect waves-light btn btn-danger remove-record" data-toggle="modal" data-url="{{route('ticket.category.destroy', $id) }}" data-id="{{$id}}" data-target="#delete-modal"  title="{{_i('Delete')}}">
