	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="header"> {{_i('Create New Category')}} </h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form id="form_add" class="form-horizontal" action="{{route('ticket.category.store')}}" method="POST" data-parsley-validate="" enctype="multipart/form-data">
					@csrf
					<div class="box-body">
						...
						<div class="form-group" >
							...
							<input type="text" class="form-control name" name="name" required="" placeholder="{{_i('Place Enter Category Name')}}">
						</div>
						<!-- ================================== title =================================== -->
						<div class="form-group">
							<label class="control-label"><?=_i('Color')?></label>
							<input type="color" class="form-control color" name="color" placeholder="{{_i('Place Enter Category Color')}}">
						</div>
						<div class="form-group">
							<label class="control-label" for="image">{{_i('Image')}}</label>
							<div>
								<input type="file" name="image" class="btn btn-default image_add" accept="image/*">
...
<div class="modal modal_edit fade" id="edit-modal">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="header"> {{_i('Edit Modal')}} </h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form id="edit-form" class="form-horizontal" method="POST" data-parsley-validate="" enctype="multipart/form-data">
					...
					<div class="box-body">
						<div class="form-group">
							<label class="control-label"><?=_i('Color')?></label>
							<input type="color" id="color" class="form-control" name="color" placeholder="{{_i('Place Enter Category Color')}}">
						</div>
						<div class="form-group">
							<label class="control-label" for="image">{{_i('Image')}}</label>
							<div>
								<input type="file" name="image" id="image" class="btn btn-default" accept="image/*" onChange="showImg(this)">
								<img src="" id='old_img'>
								<span class="text-danger invalid-feedback">
									<strong>{{$errors->first('image')}}</strong>
								</span>
...
@push('js')
	<script>
		...
		$('body').on('click', '.edit', function (e) {
			...
			$("#edit-modal #old_img").attr('src',"{{asset('')}}/"+image);
		});
	...
	<div class="modal-dialog" role="document">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form  action="{{route('ticket.category.store_translation')}}" method="post" class="form-horizontal"  id="lang_submit" data-parsley-validate="">
						@csrf
						...
						<div class="box-body">
							...
							<div class="form-group row">
								...
								<div class="col-sm-10">
									<input type="text" id="name" class="form-control" name="name" required="" placeholder="{{_i('Place Enter category Name')}}">
								</div>
							</div>
							<!----============================== content =============================-->
							<div class="form-group row">
