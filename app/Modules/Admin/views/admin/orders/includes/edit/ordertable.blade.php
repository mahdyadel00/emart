        ...
        <div class="card-body">
            ...
            <table class="table">
                ...
                @if(count($order->orderProducts) > 0)
                    ...
                    @foreach($order->orderProducts as $item)
                        ...
                        <tr>
                            <td data-th="Product">
                                <div class="row">
                                    <div class="col-sm-3 hidden-xs"><img src="{{ asset($item->product->mainPhoto()) }}"
                                                                         width="100" height="100"
                                                                         class="img-responsive"/></div>
                                    <div class="col-sm-9">
                                        <h4 class="nomargin">{{ ($item->product->product_details()->first() != null) ? $item->product->product_details()->first()->title : "" }}</h4>
                                    </div>
                                </div>
                            </td>
                            <td data-th="Price">{{ apply_discount_code_on_price($item->price, $item->discount, 'perc')['price'] }}</td>
                            <td data-th="Quantity">
                                <input type="number" name="product[{{ $item->product_id }}][quantity]"
                                       data-price="{{ apply_discount_code_on_price($item->price, $item->discount, 'perc')['price'] }}" value="{{ $item->count }}"
                                       min="1"
                                       class="quantity"/>
                            </td>
                            <td data-th="Subtotal" class="text-center Subtotal">{{ apply_discount_code_on_price($item->price, $item->discount, 'perc')['price'] * $item->count }}</td>
                        ...
                        @else
                        <p align="center">Product Not Found Or Deleted</p>
                        @endif
                    @endforeach
                ...
                <tr class="productRowOne">
                    ...
                    <?php
                   	$cost = ($order->shipping_cost==null )? "0.0": $order->shipping_cost ;
                    ?>
                    <td class="Shipping__cost">{{ $cost }}</td>
                    <td></td>
                </tr>
                <tr class="productRowOne">
        ...
        $(function () {
            ...
            $('body').on('click', '.remove-from-cart', function (e) {
                ...
                var product_id = $(this).data('id');
                 ...
                 var form_data ={order_id:order_id , product_id : product_id};
