...
@section('content')
<div class="box-body">country
	<div class="row">
		<div class="col-sm-12">
			<div class="card">
				<div class="card-header">
					<h5>{{_i('All regions')}}</h5>
						...
						<table id="countries_table" class="table table-bordered table-striped dataTable text-center">
							<thead>
								<tr role="row">
									...
									<th class="text-center">{{_i('city')}}</th>
...
<script  type="text/javascript">
	$(function () {
	    var table = $('#countries_table').DataTable({
	        processing: true,
	        serverSide: true,
	        ajax: "{{ route('regions.show', $city->id) }}",
	        columns: [
	            {data: 'id'},
	            {data: 'title', name:'region_data.title'},
	            {data: 'ctitle', name: 'city_datas.title', searchable: false},
	            {data: 'status'},
	        ]
		});
...
$(document).on('change', '.change_status_city', function() {
		var url = '{{route('change.status.region.and.store')}}';
